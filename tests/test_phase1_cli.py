...
class Phase1SetupTests(unittest.TestCase):
    ...
    def test_setup_repo_creates_phase1_files(self) -> None:
        ...
        self.assertIn("repositories hosted on GitHub", agents)
        self.assertIn("GitHub server-side `main` branch protection", agents)
        self.assertIn("merged into `main` only through pull requests", agents)
        self.assertIn("Do not push directly to `main`", agents)
        self.assertIn("server-side `main` branch protection", agents)
        self.assertIn("For non-GitHub hosted repositories", agents)

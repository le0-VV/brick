...
def run_rebuild(repo: Path, *args: str) -> subprocess.CompletedProcess[str]:
...
def run_search(repo: Path, query: str, *args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
...
class Phase4IndexSearchTests(unittest.TestCase):
    def test_rebuild_creates_sqlite_index_and_json_contract(self) -> None:
        ...
        self.assertEqual(payload["index"]["schema_version"], 1)

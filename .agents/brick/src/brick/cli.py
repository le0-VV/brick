...
def brick_agents_text(has_backup: bool) -> str:
    ...
    return (
        ...
        "`embedding.model`. If either field is blank, ask the user whether they "
        "want semantic retrieval for this machine. If they opt in, ask for the "
        "embedding server URL and model name, write both values to the local "
        "config file, and run `./brick rebuild`. If they opt out, report that "
        "Brick search is keyword-only.\n"
